...
public class ExtratoBean {
	...
	public void validaDatas(ComponentSystemEvent event) {
		...
		if (dataInicialInput.isValid() && dataFinalInput.isValid()) {
			dataInicial = (Date) dataInicialInput.getLocalValue();
			dataFinal = (Date) dataFinalInput.getLocalValue();
			if (dataFinal.before(dataInicial)) {
				FacesMessage message = new FacesMessage("A data final não pode vir antes da inicial..");
